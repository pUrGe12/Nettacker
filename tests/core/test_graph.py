...
class TestGraphModule(TestCase):

    @patch("nettacker.core.graph.importlib.import_module")
    def test_build_graph_success(self, mock_import_module):
        ...
        self.assertEqual(result, "<graph_html>")
    ...
    def test_build_graph_module_not_found(self, mock_import_module, mock_die_failure):
        build_graph("missing_graph", [])
        mock_die_failure.assert_called_once()

    @patch("nettacker.core.graph.importlib.import_module")
    def test_build_compare_report_success(self, mock_import_module):
        ...
        self.assertEqual(result, "<compare_html>")

    @patch("nettacker.core.graph.die_failure")
    @patch("nettacker.core.graph.importlib.import_module", side_effect=ModuleNotFoundError)
    @pytest.mark.xfail(reason="It will hit an UnboundLocalError")
    def test_build_compare_report_module_not_found(self, mock_import_module, mock_die_failure):
    ...
    def test_build_text_table(self, mock_now, mock_version_info, mock_merge_logs):
        events = [{
            "date": "today",
            "target": "127.0.0.1",
    ...
    def test_create_compare_text_table(self):
        results = {
            "A": "value1",
            "B": "value2"
        }
    ...
    def test_no_events(self, mock_log_info, mock_get_logs):
        options = MagicMock()
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("nettacker.core.graph.submit_report_to_db")
    def test_json_report(self, mock_submit, mock_open_file, mock_get_logs):
        options = MagicMock()
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("nettacker.core.graph.submit_report_to_db")
    def test_csv_report(self, mock_submit, mock_open_file, mock_csv_writer, mock_get_logs):
        options = MagicMock()
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("nettacker.core.graph.submit_report_to_db")
    def test_text_report(self, mock_submit, mock_open_file, mock_build_text, mock_get_logs):
        options = MagicMock()
    ...
    def test_html_json_csv_text(
        self, mock_text_table, mock_join, mock_norm, mock_open_file,
        mock_build_html, mock_get_opts, mock_get_logs
    ):
    ...
    @patch("nettacker.core.graph.get_logs_by_scan_id", return_value=[])
    def test_no_current_logs(self, mock_logs):
        result = create_compare_report(DummyOptions("scan-comp", "report.html"), "scan-1")
        self.assertIsNone(result)

    @patch("nettacker.core.graph.get_logs_by_scan_id")
    def test_no_comparison_logs(self, mock_logs):
        def logs_side_effect(scan_id):
            return [{"date": "now", "target": "x", "module_name": "mod", "port": 80, "options": json.dumps({"targets": ["x"]})}] if scan_id == "scan-1" else []
    ...
    def test_permission_error(self, mock_join, mock_norm, mock_opts, mock_logs):
        dummy_log = {
            "target": "1.1.1.1",
            "module_name": "mod",
            "port": 80,
    ...
    @patch("nettacker.core.graph.open", new_callable=mock_open)
    def test_dict_options(self, mock_open_file, mock_text, mock_opts, mock_logs):
        dummy_log = {
            "target": "1.1.1.1",
            "module_name": "mod",
            "port": 80,
